...
const Comments = () => {
    ...
    const onClickCancel = (comment) => {
        if(comment.contents !== comment.text){
            if(!window.confirm('정말로 취소하시겠어요?')) return;
        }
    ...
    <div className='my-5'>
        {!email ?
            <div className='text-end'>
                <Button className='px-5' onClick={onClickInsert}>댓글등록하기</Button>
            </div>
        :
            <div>
                <Form.Control value={contents} onChange={(e) => {setContents(e.target.value)}}
                as='textarea' rows={2} placeholder='댓글을 입력하세요!'/>
                <div className='text-end'>
                    <Button className='px-3 my-3' onClick={onInsert}>댓글등록</Button>
                </div>
            </div>
        }
        <div className='my-5'>
            {comments.map(com =>
                <div key={com.id}>
                    ...
                    <Form.Control onChange={(e) => onChangeContents(e, com.id)}
                        value={com.contents} as='textarea' rows={2}/>
                    : 
